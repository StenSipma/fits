...
pub mod parsing {
    ...
    use crate::{definitions, HeaderList, RawHeaderList};

    pub fn print_header(header: &Vec<header::Keyword>) {
    ...
    pub mod header {
        ...
        impl Keyword {
            pub fn print(&self) {
                ...
                match self {
                    Keyword::Value(kw, value, comment) => println!("{:8} | {:>30} / {}", kw, value, comment),
                    Keyword::Continue(kw, value, comment) => println!("{:8} | {:>30} / {}", kw, value, comment),
        ...
        impl<'a> HeaderChunk<'a> {
            pub fn print(&self) {
                ...
                match self {
                    ...
                    HeaderChunk::History(v)          => println!("{:8} {:>30}", definitions::HEADER_HISTORY_KEYWORD, v),
                    HeaderChunk::Comment(v)          => println!("{:8} {:>30}", definitions::HEADER_COMMENT_KEYWORD, v),
        ...
        fn split_header_chunk<'a>(header_chunk: &'a str) -> (&'a str, &'a str, &'a str) {
            ...
            if &header_chunk[name_idx..sep_idx] == definitions::HEADER_VALUE_INDICATOR {
                (&header_chunk[..name_idx], &header_chunk[name_idx..sep_idx], &header_chunk[sep_idx..])
            } else {
        ...
        pub fn parse_header<'a>(blocks: &mut Chunks<'a, u8>) -> HeaderList {
                    ...
                    Keyword::Value(kw, v0, c0) if kw == continue_kw && v0.is_str() => {
                        match header.pop() {
                             ...
                             Some(prev) => {
                                 ...
                                 // as keyword.
                                 header.push(prev);
                                 header.push(Keyword::Continue(definitions::HEADER_CONTINUE_KEYWORD.to_string(), v0, c0));
                             },
                             None => header.push(Keyword::Continue(definitions::HEADER_CONTINUE_KEYWORD.to_string(), v0, c0)),
                        }
                    },
                    kw => header.push(kw),
                }
        ...
        fn find_value<'a, 'b>(header: &'a HeaderList, key: &'b str) -> Option<Value> {
        ...
        mod tests {
            ...
            fn parse_from_bytes_test() {
                ...
                assert_eq!(res.unwrap(), HeaderChunk::RawValue("SIMPLE", "T / conforms to FITS standard"));
                ...
                let keyword_bytes = Vec::from_iter(keyword.bytes());
                let res = HeaderChunk::from_bytes(&keyword_bytes);
                assert_eq!(res.unwrap(), HeaderChunk::Comment("This is a comment, and therefore does not have a value separator."));
                ...
                let keyword_bytes = Vec::from_iter(keyword.bytes());
                let res = HeaderChunk::from_bytes(&keyword_bytes);
                assert_eq!(res.unwrap(), HeaderChunk::RawValue("KEYWORD", "'something with a /       ' / and also a comment"));
    ...
    pub fn read_fits_buffer<'a>(buffer: &'a Vec<u8>) -> (HeaderList, Option<Vec<f64>>) {
        ...
        let (_naxis, axes, bitpix) = header::extract_values(&header);
...
type HeaderList = Vec<parsing::header::Keyword>;
...
impl Fits {
    pub fn open<'a>(filename: String) -> Option<(HeaderList, Option<Vec<f64>>)> {
        let mut f = File::open(filename).unwrap();

        let mut buffer = Vec::new();

        if let Ok(_) = f.read_to_end(&mut buffer) {
            let (h, b) = parsing::read_fits_buffer(&buffer);
            Some((h, b))
        } else {
            None
        }
