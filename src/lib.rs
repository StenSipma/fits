...
mod parsing {
    ...
    fn print_header(header: &Vec<Keyword>) {
    ...
    impl Value {
        pub fn from_str(value: &str) -> Self {
            ...
            // ASCII for T and F respectively
            let t = 84;
            let f = 70;
            if value_bytes[0] == t || value_bytes[0] == f {
                return Value::Boolean(value_bytes[0] == t);
            }
    ...
    fn parse_header<'a>(blocks: &mut Chunks<'a, u8>) -> Vec<Keyword<'a>> {
        let mut header: Vec<Keyword> = Vec::new();
    ...
    fn find_raw_value<'a, 'b>(header: &Vec<Keyword<'a>>, key: &'b str) -> Option<&'a str> {
    ...
    fn extract_values(header: &Vec<Keyword>) -> (usize, Vec<usize>, i64) {
        let naxis = {
            let raw_naxis = find_raw_value(header, "NAXIS").unwrap();
            if let Value::Integer(i) = Value::from_str(raw_naxis) {
                i as usize
            } else {
        ...
        let bitpix = {
            let raw_bitpix = find_raw_value(header, "BITPIX").unwrap();
            if let Value::Integer(i) = Value::from_str(raw_bitpix) {
                i
            } else {
        ...
        for i in 1..=naxis {
            ...
            let kw = kw.as_str();
            let raw_axis = find_raw_value(header, kw).unwrap();
            if let Value::Integer(i) = Value::from_str(raw_axis) {
    ...
    pub fn read_fits_buffer(buffer: Vec<u8>) {
        ...
        for h in header.iter() {
            if let Keyword::RawValue(kw, v) = h {
                let value = Value::from_str(v);
                println!("{:8} {}", kw, value);
            }
        }

        let (naxis, axes, bitpix) = extract_values(&header);
        println!("Extracted: ");
        println!("NAXIS {}", naxis);
        ...
        if bitpix == -64 {
            ...
            // maybe usefull for abstracting: https://stackoverflow.com/questions/38168956/take-slice-of-certain-length-known-at-compile-time
            let mut data: Vec<f64> = Vec::with_capacity(axes.iter().product::<usize>());
            let mut rem = bytes;
            while rem > 0 {
                let block = blocks.next().unwrap();
...
impl Fits {
    pub fn open(filename: String) {
        let mut f = File::open(filename).unwrap();

        let mut buffer = Vec::new();

        if let Ok(_) = f.read_to_end(&mut buffer) {
            parsing::read_fits_buffer(buffer);
        }
    }
}
